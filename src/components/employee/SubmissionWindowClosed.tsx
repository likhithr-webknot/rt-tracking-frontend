...
import { Calendar, Clock, Lock, LogOut, ShieldAlert } from "lucide-react";
...
export default function SubmissionWindowClosed({
  portalWindow,
  globalOpen = null,
  roleOpen = null,
  portalLabel = "Portal",
  ...
  const start = parseDateTime(
    portalWindow?.startAt ?? portalWindow?.windowStartAt ?? portalWindow?.start,
  );
  const end = parseDateTime(portalWindow?.endAt ?? portalWindow?.windowEndAt ?? portalWindow?.end);
  const endValue = portalWindow?.endAt ?? portalWindow?.windowEndAt ?? portalWindow?.end;
