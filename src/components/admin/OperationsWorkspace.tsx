...
import * as legacyOps from "../../api/portal-workflows";
...
function useRunner() {
  ...
  async function run(label, fn, { downloadName } = {} as ApiOptions) {
    setBusy(label);
    setResult(null);
    try {
      const data = await fn();
...
export default function OperationsWorkspace() {
  const domains = useMemo(() => ([
    ...
    { id: "legacy", label: "Legacy v1", icon: GitBranch },
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
        <div>
          ...
          <p className="mt-2 max-w-3xl text-sm text-[rgb(var(--muted))]">
            ...
            requests, reports, uploads, notifications, cron triggers, reference lists, and older singular-path endpoints (Legacy v1 tab).
          </p>
        </div>
      </div>
        ...
        <div className="space-y-4">
            ...
            <>
              ...
              <ActionCard title="User Mutations" description="Create users, assign roles, update leave details, or update employee profile data." icon={UserCog}>
                ...
                <label className="mt-3 block">
                  <span className="mb-1 block text-xs font-semibold text-[rgb(var(--muted))]">Batch user file (for user batch)</span>
                ...
                <div className="mt-3 flex flex-wrap gap-2">
                  ...
                  <ActionButton busy={isBusy("PUT /profile")} icon={Send} onClick={() => run("PUT /profile", () => operations.users.updateProfile(parsedPayload()))}>Update Profile</ActionButton>
                  ...
                  <ActionButton busy={isBusy("POST user batch")} icon={FileUp} onClick={() => run("POST user batch", () => operations.users.batch(file, parsedPayload()))}>User Batch</ActionButton>
                </div>
              </ActionCard>
            </>
          ) : null}
          ...
          {active === "projects" ? (
            <ActionCard title="Project Directory" description="Create projects, inspect project directories, and view manager project assignments." icon={FolderKanban}>
              ...
              <div className="mt-3 flex flex-wrap gap-2">
                ...
                <ActionButton busy={isBusy("GET manager projects + roles")} icon={Users} onClick={() => run("GET manager projects + roles", () => operations.projects.managerProjectsWithRoles())}>Manager Projects + Roles</ActionButton>
          ...
          {active === "requests" ? (
            <ActionCard title="User Requests" description="Create leave/allocation requests, review queues, status changes, reminders, and manager lookups." icon={ClipboardList}>
              ...
              <label className="mt-3 block">
                <span className="mb-1 block text-xs font-semibold text-[rgb(var(--muted))]">Optional file (create / update request)</span>
                <input type="file" onChange={(e) => setFile(e.target.files?.[0] || null)} className="rt-input w-full px-3 py-2 text-sm" />
              </label>
              ...
              <div className="mt-3 flex flex-wrap gap-2">
                ...
                <ActionButton busy={isBusy("PUT request update")} icon={Send} onClick={() => run("PUT request update", () => operations.requests.update(parsedPayload(), file))}>Update Request</ActionButton>
          ...
          {active === "reference" ? (
            <ActionCard title="Reference Lists" description="Load departments, streams, designation lookups, band lists, and leave summary." icon={Search}>
              <div className="grid gap-3 sm:grid-cols-2">
              ...
              <div className="mt-3 flex flex-wrap gap-2">
                ...
                <ActionButton busy={isBusy("POST leave transaction")} icon={Send} onClick={() => run("POST leave transaction", () => operations.leave.createTransaction(parsedPayload()))}>Leave Transaction</ActionButton>
              </div>
            </ActionCard>
          ) : null}
          ...
          {active === "legacy" ? (
            <>
              <ActionCard title="Legacy wildcard paths" description="Trailing segment after /api/v1/privateKey/**, secret-code/**, notifications/**, timelog/get/**, userRequest/**, report/manager/**, export/timelogs/** (no leading slash)." icon={GitBranch}>
                <TextField label="Path suffix" value={pathSuffix} onChange={setPathSuffix} placeholder="e.g. subscribe/USER_ID or get/email/from/to/TYPE" />
                <div className="mt-3 flex flex-wrap gap-2">
                  <ActionButton busy={isBusy("legacy GET privateKey")} icon={Search} onClick={() => run("legacy GET privateKey", () => legacyOps.fetchPrivateKey(pathSuffix))}>Private Key</ActionButton>
                  <ActionButton busy={isBusy("legacy POST secret-code")} icon={Send} onClick={() => run("legacy POST secret-code", () => legacyOps.createSecretCode(pathSuffix, parsedPayload()))}>Secret Code POST</ActionButton>
                  <ActionButton busy={isBusy("legacy GET secret-code")} icon={Search} onClick={() => run("legacy GET secret-code", () => legacyOps.fetchSecretCode(pathSuffix))}>Secret Code GET</ActionButton>
                  <ActionButton busy={isBusy("legacy GET notifications")} icon={Search} onClick={() => run("legacy GET notifications", () => legacyOps.fetchPortalNotifications(pathSuffix))}>Notifications GET</ActionButton>
                  <ActionButton busy={isBusy("legacy PUT notifications")} icon={Send} onClick={() => run("legacy PUT notifications", () => legacyOps.updatePortalNotifications(pathSuffix, parsedPayload()))}>Notifications PUT</ActionButton>
                  <ActionButton busy={isBusy("legacy GET timelog")} icon={Search} onClick={() => run("legacy GET timelog", () => legacyOps.fetchTimelog(pathSuffix))}>Timelog GET</ActionButton>
                  <ActionButton busy={isBusy("legacy GET userRequest")} icon={Search} onClick={() => run("legacy GET userRequest", () => legacyOps.fetchUserRequest(pathSuffix))}>UserRequest GET</ActionButton>
                  <ActionButton busy={isBusy("legacy GET userRequest createDate")} icon={Search} onClick={() => run("legacy GET userRequest createDate", () => legacyOps.fetchUserRequestByCreateDate(pathSuffix))}>UserRequest By Create</ActionButton>
                  <ActionButton busy={isBusy("legacy GET userRequest forDate")} icon={Search} onClick={() => run("legacy GET userRequest forDate", () => legacyOps.fetchUserRequestForDate(pathSuffix))}>UserRequest For Date</ActionButton>
                  <ActionButton busy={isBusy("legacy GET manager report")} icon={Search} onClick={() => run("legacy GET manager report", () => legacyOps.fetchManagerReport(pathSuffix))}>Manager Report</ActionButton>
              ...
              <ActionCard title="Legacy JSON bodies" description="Singular /timelog routes and admin POST helpers used by older backends." icon={Database}>
                ...
                <div className="mt-3 flex flex-wrap gap-2">
                  ...
                  <ActionButton busy={isBusy("legacy POST projects")} icon={Send} onClick={() => run("legacy POST projects", () => legacyOps.createAdminProject(parsedPayload()))}>Admin POST Projects</ActionButton>
                  <ActionButton busy={isBusy("legacy POST allocation batch JSON")} icon={Send} onClick={() => run("legacy POST allocation batch JSON", () => legacyOps.createAllocationBatch(parsedPayload()))}>Alloc Batch JSON</ActionButton>
