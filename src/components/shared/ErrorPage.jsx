...
/**
 * Reusable full-page error screen — visually matches NotFoundPage.
 *
 * @param {{ code?: string|number, title?: string, message?: string, error?: Error|null, onGoHome?: () => void }} props
 */
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-[rgb(var(--bg))] px-6 py-12 overflow-hidden relative select-none">
      ...
      <div className="absolute inset-0 pointer-events-none overflow-hidden">
        <motion.div
          className="absolute top-[10%] left-[15%] w-80 h-80 rounded-full bg-red-500/6 blur-[100px]"
          animate={{ x: [0, 40, 0], y: [0, -25, 0], scale: [1, 1.1, 1] }}
          transition={{ duration: 10, repeat: Infinity, ease: "easeInOut" }}
        />
        <motion.div
          className="absolute bottom-[15%] right-[12%] w-[28rem] h-[28rem] rounded-full bg-amber-500/5 blur-[120px]"
          animate={{ x: [0, -30, 0], y: [0, 20, 0], scale: [1, 1.05, 1] }}
          transition={{ duration: 12, repeat: Infinity, ease: "easeInOut" }}
        />
        <motion.div
          className="absolute top-[55%] left-[50%] w-64 h-64 rounded-full bg-purple-500/4 blur-[80px]"
      ...
      >
        ...
        <motion.div className="flex justify-center mb-6" variants={scaleIn}>
          ...
          >
            <motion.div
              ...
              style={{
                background: "radial-gradient(circle, rgb(239 68 68 / 0.15) 0%, transparent 70%)",
              }}
              animate={{ scale: [1, 1.15, 1], opacity: [0.5, 0.8, 0.5] }}
              transition={{ duration: 3, repeat: Infinity, ease: "easeInOut" }}
            />
            <div className="relative w-28 h-28 sm:w-32 sm:h-32 rounded-full border-2 border-[rgb(var(--border))]/50 bg-[rgb(var(--surface))] shadow-xl shadow-black/10 flex items-center justify-center overflow-hidden">
              <motion.img
                src="/unnamed.webp"
                alt="Webknot"
        ...
        <motion.div variants={fadeUp} className="relative inline-block">
          <motion.h1
            className="text-[7rem] sm:text-[9rem] font-black tracking-tighter leading-none bg-gradient-to-br from-[rgb(var(--text))] via-red-500 to-amber-500 bg-clip-text text-transparent"
          ...
          <svg className="absolute -bottom-1 left-1/2 -translate-x-1/2 w-36 h-3" viewBox="0 0 160 12" fill="none">
            ...
            />
            <defs>
              <linearGradient id="err-ul-grad" x1="0" y1="0" x2="160" y2="0" gradientUnits="userSpaceOnUse">
        ...
        {errorDetail ? (
          <motion.div
            variants={fadeUp}
            className="mt-5 mx-auto max-w-md rounded-lg border border-red-500/20 bg-red-500/5 px-4 py-3 text-xs text-left text-red-400 font-mono break-words"
          >
            {errorDetail}
          </motion.div>
        ) : null}
