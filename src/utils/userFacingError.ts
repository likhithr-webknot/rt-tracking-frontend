...
const STATUS_FALLBACKS: Record<number, string> = {
  ...
  504: "The service is temporarily unavailable. Please try again in a moment.",
  401: "Your session expired. Please sign in again.",
  403: "You don't have permission to view this data.",
};

export function userFacingMessageForStatus(status: unknown, fallback = USER_FACING_ERROR_FALLBACK) {
  const code = Number(status);
  if (Number.isFinite(code) && STATUS_FALLBACKS[code]) return STATUS_FALLBACKS[code];
  return fallback;
}
