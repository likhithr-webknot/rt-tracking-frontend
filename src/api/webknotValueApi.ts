...
export async function fetchValues(activeOnly = true, { limit = null, cursor = null, offset = null, signal } = {} as ApiOptions) {
...
export async function addValue(data, { signal } = {} as ApiOptions) {
    const auth = getAuthHeader();
    ...
    const endpoints = ["/api/v1/webknot-value/add", "/webknot-value/add"];
    let lastRouteErr: Error | null = null;

    for (const endpoint of endpoints) {
        const postOnce = () =>
            fetch(buildWebknotValueUrl(endpoint), {
                method: "POST",
                signal,
                credentials: "include",
                headers: jsonHeaders(),
...
export async function updateValue(id, data) {
    ...
    const singular = `/api/v1/webknot-value`;
    const plural = `/api/v1/webknot-values`;
    const endpoints = [
        { method: "PUT", path: `${singular}/${safeId}` },
        { method: "PATCH", path: `${singular}/${safeId}` },
        { method: "POST", path: `${singular}/${safeId}` },
        { method: "PUT", path: `${singular}/update/${safeId}` },
        { method: "PATCH", path: `${singular}/update/${safeId}` },
        { method: "POST", path: `${singular}/update/${safeId}` },
        { method: "PUT", path: `${singular}/edit/${safeId}` },
        { method: "PATCH", path: `${singular}/edit/${safeId}` },
        { method: "POST", path: `${singular}/edit/${safeId}` },
        { method: "PUT", path: `${plural}/${safeId}` },
        { method: "PATCH", path: `${plural}/${safeId}` },
        { method: "POST", path: `${plural}/${safeId}` },
        { method: "PUT", path: `${plural}/update/${safeId}` },
        { method: "PATCH", path: `${plural}/update/${safeId}` },
        { method: "POST", path: `${plural}/update/${safeId}` },
        { method: "PUT", path: `${plural}/edit/${safeId}` },
        { method: "PATCH", path: `${plural}/edit/${safeId}` },
        { method: "POST", path: `${plural}/edit/${safeId}` },
    ];
    let lastRouteErr = null;
...
export async function deleteValue(id) {
    ...
    const endpoints = [
        `/api/v1/webknot-value/${safeId}`,
        `/api/v1/webknot-value/delete/${safeId}`,
        `/api/v1/webknot-values/${safeId}`,
        `/api/v1/webknot-values/delete/${safeId}`,
    ];
    let lastRouteErr = null;
    for (const endpoint of endpoints) {
        const res = await fetch(buildWebknotValueUrl(endpoint), {
