...
import { extractEvaluationCriteria } from "../utils/evaluationCriteria";
import { getAuthHeader } from "./auth";
import { buildApiUrl, ensureCsrfCookie, parseResponse, toHttpError, withCsrfHeaders } from "./http";

/**
 * KPI definitions / KPI directions (controllers may use either name, base `/api/v1/`).
 * List: GET `kpi-directions` | `list-kpi-directions` | `kpi-definitions` | `list-kpi-definitions` (?limit, ?offset)
 * One: GET `kpi-directions/{id}` | `get-kpi-direction/{id}` | `kpi-definitions/{id}` | `get-kpi-definition/{id}`
 * Create: POST `add-kpi-definition` | `add-kpi-direction` first (avoids ambiguous `POST kpi-definitions` when the API maps add+update to the same path), then `kpi-directions` | `kpi-definitions`
 * Update: same verbs for `kpi-directions/{id}` + legacy `update-kpi-direction*`, `edit-kpi-direction*`, plus definition equivalents
 * Delete: DELETE `kpi-directions/{id}` | `delete-kpi-direction/{id}` | definition equivalents
 */
...
function toAddRequestBody(input) {
  ...
  const streamText = normalizeRelationText(obj.stream ?? obj.department ?? obj.context ?? null);
  const bandText = normalizeRelationText(obj.band ?? obj.level ?? null);
  const bandId =
    parsePositiveLongId(obj.bandId) ??
    parsePositiveLongId(obj.band_id) ??
  ...
  // JPA maps `Band band` on the entity; flat `bandId` alone is not bound unless the API supports it.
  // Sending a reference object is the reliable shape for Jackson + Hibernate.
  if (bandId != null) {
    body.bandId = bandId;
    body.band = { id: bandId };
  } else if (bandText) {
    body.band = { name: bandText };
  }
  return body;
}
...
export async function addKpiDefinition(payload, { signal } = {} as ApiOptions) {
  ...
  // Prefer explicit add-* URLs: Spring may map both add and update-by-body to POST /kpi-definitions (ambiguous).
  const paths = [
    "/api/v1/add-kpi-definition",
    "/api/v1/add-kpi-direction",
    "/api/v1/kpi-directions",
  ...
  for (const path of paths) {
    ...
    if (res.status === 404 || res.status === 405) {
      lastRouteErr = await toHttpError(res);
      continue;
    }
    throw await toHttpError(res);
  }
  throw lastRouteErr || new Error("KPI create endpoint not found.");
}
...
async function fetchKpiDefinitionsPage({ safeLimit, safeOffset, signal }) {
  ...
  const auth = getAuthHeader();
  const baseHeaders = auth ? { Authorization: auth } : undefined;
  ...
  throw lastRouteErr || new Error("KPI list endpoint not found.");
}

/**
 * Lists KPI definitions (GET with limit + offset).
 * Tries `kpi-directions` / `list-kpi-directions` first, then `kpi-definitions` / `list-kpi-definitions`.
 * The UI pager passes `cursor`; it is sent as `offset`.
 * With `limit: null`, fetches all pages from offset 0 (for lookup maps).
 */
export async function fetchKpiDefinitions({ limit = null, cursor = null, offset = null, signal } = {} as ApiOptions) {
...
export async function fetchKpiDefinition(id, { signal } = {} as ApiOptions) {
  ...
  const paths = [
    `/api/v1/kpi-directions/${safeId}`,
    `/api/v1/get-kpi-direction/${safeId}`,
    `/api/v1/kpi-definitions/${safeId}`,
    `/api/v1/get-kpi-definition/${safeId}`,
  ];
  ...
  throw lastRouteErr || new Error("KPI get endpoint not found.");
}

export async function updateKpiDefinition(payload, { signal } = {} as ApiOptions) {
  ...
  const endpoints = [
    { method: "PATCH", path: `/api/v1/kpi-directions/${safeId}` },
    { method: "PUT", path: `/api/v1/kpi-directions/${safeId}` },
    { method: "POST", path: `/api/v1/kpi-directions/${safeId}` },
    { method: "PATCH", path: `/api/v1/update-kpi-direction/${safeId}` },
    { method: "PUT", path: `/api/v1/update-kpi-direction/${safeId}` },
    { method: "POST", path: `/api/v1/update-kpi-direction/${safeId}` },
    { method: "PATCH", path: `/api/v1/edit-kpi-direction/${safeId}` },
    { method: "PUT", path: `/api/v1/edit-kpi-direction/${safeId}` },
    { method: "POST", path: `/api/v1/edit-kpi-direction/${safeId}` },
    { method: "PATCH", path: "/api/v1/kpi-directions" },
    { method: "PUT", path: "/api/v1/kpi-directions" },
    { method: "POST", path: "/api/v1/kpi-directions" },
    { method: "PUT", path: "/api/v1/update-kpi-direction" },
    { method: "POST", path: "/api/v1/update-kpi-direction" },
    { method: "PATCH", path: `/api/v1/kpi-definitions/${safeId}` },
    { method: "PUT", path: `/api/v1/kpi-definitions/${safeId}` },
    { method: "POST", path: `/api/v1/kpi-definitions/${safeId}` },
    ...
    { method: "PATCH", path: "/api/v1/kpi-definitions" },
    { method: "PUT", path: "/api/v1/kpi-definitions" },
    // Omit POST /api/v1/kpi-definitions: same path as addKpiDefinition → ambiguous handler on some APIs.
...
export async function deleteKpiDefinition(id, { signal } = {} as ApiOptions) {
  const safeId = encodeURIComponent(String(id ?? "").trim());
  ...
  const paths = [
    `/api/v1/kpi-directions/${safeId}`,
    `/api/v1/delete-kpi-direction/${safeId}`,
    `/api/v1/kpi-definitions/${safeId}`,
    `/api/v1/delete-kpi-definition/${safeId}`,
  ];
  let lastRouteErr = null;
